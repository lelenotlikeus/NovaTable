...
export function CardArtworkPicker({ cardName, onClose, onSelect }: { cardName: string; onClose: () => void; onSelect?: (printing: CardPrinting | null) => void }) {
  ...
  return <div className="artwork-picker-backdrop" role="dialog" aria-modal="true" aria-label={`Choose artwork for ${cardName}`}>
    <section className="artwork-picker">
      ...
      <div className="artwork-picker-actions"><button onClick={() => { setPreferredCardPrinting(cardName, null); onSelect?.(null); onClose(); }}><RotateCcw size={14} />Use default artwork</button><span>{printings.length ? `${printings.length} printings` : "Official printings"}</span></div>
      {loading && <div className="artwork-picker-status"><LoaderCircle className="spin" size={24} />Loading editions and artwork…</div>}
      {error && <div className="inline-error">{error}</div>}
      {!loading && !error && <div className="artwork-grid">{printings.map((printing) => <button className={current?.printingId === printing.printingId ? "is-current" : ""} key={printing.printingId} onClick={() => { setPreferredCardPrinting(cardName, printing); onSelect?.(printing); onClose(); }}>

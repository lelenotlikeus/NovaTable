...
export function CommanderBoard({
  ...
  const devTurnHandoff = state.activePlayerId.startsWith("bot-") || nextPlayerId.startsWith("bot-");
  const canAdvancePhase = state.phase !== "end" || nextPlayerId === localId || devTurnHandoff;
...
function OpponentBoard({ state, playerId, dispatch, onHover, onContext, onViewZone }: { state: CommanderGameState; playerId: string; dispatch: (action: CommanderGameAction) => void; onHover: (cardId: string | null) => void; onContext: (card: CommanderCardState, event: MouseEvent<HTMLElement>) => void; onViewZone: (zone: "graveyard" | "exile") => void }) {
  ...
  return <section className={`opponent-board ${state.activePlayerId === playerId ? "is-active" : ""}`}>
    ...
    <div className="opponent-public">
      <div className="opponent-battlefield">{!battlefield.length && <span className="opponent-battlefield-empty">Battlefield</span>}{battlefield.map((card, index) => <CommanderCard key={card.id} card={card} selected={false} freePosition={battlefieldPlacement(card, battlefield, [18 + index * 18, 50])} onHover={(hovered) => onHover(hovered ? card.id : null)} onContext={(event) => onContext(card, event)} onDouble={() => dispatch({ type: card.tapped ? "UNTAP_CARD" : "TAP_CARD", cardId: card.id })} />)}<div className="opponent-hand-count" title={revealedHand.length ? `${hand.length} cards · revealed: ${revealedHand.map((card) => card.name).join(", ")}` : `${hand.length} cards in hand`} onMouseEnter={() => revealedHand.length && onHover(revealedHand.at(-1)!.id)} onMouseLeave={() => revealedHand.length && onHover(null)}><Hand size={14} /><span>Hand</span><strong>{hand.length}</strong>{revealedHand.length > 0 && <b>{revealedHand.length} shown</b>}</div></div>
    </div>
  </section>;
}

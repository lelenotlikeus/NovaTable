...
import { searchCardDatabase, type CardRecord } from "../../infrastructure/cardDatabase";
import {
...
import {
  ...
  onlineLobby, onlineLobbyAction, onlinePublicLobbies
...
export function PlatformApp({
  ...
  onStartGame: (players: GameSetupPlayer[], startingLife: number, lobbyName: string, gameId: string) => void;
}) {
        ...
        <LobbyRoom
          ...
          onStart={(lobby) => {
            ...
            const players: GameSetupPlayer[] = lobby.players.map((player) => ({
              ...
              commander: player.commander ?? localDeck.commander,
              cards: player.cards?.length ? player.cards : localDeck.cards,
              local: player.userId === user.id
            }));
            onStartGame(players, lobby.startingLife, lobby.name, lobby.id);
          }}
        />
  ...
  return <div className="page-content">
    ...
    <div className="home-grid">
      <section className="card-panel section-card"><SectionTitle title="Friends online" action="View all" /><div className="people-list">
        {social.friends.filter((friend) => friend.presence !== "offline").length ? social.friends.filter((friend) => friend.presence !== "offline").map((friend) => <PersonRow key={friend.id} user={friend} />) : <EmptyMini text="Accept Mira's request to start your friends list." />}
      </div></section>
...
function CreateLobbyView({ user, onCancel, onCreated }: { user: UserProfile; onCancel: () => void; onCreated: (lobby: Lobby) => void }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); try { onCreated(await onlineCreateLobby(user, { name, format, privacy, maxPlayers, spectatorsAllowed: spectators, startingLife, password, description })); } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } }
...
function ProfileView({ user, onLogout, onUpdated }: { user: UserProfile; onLogout: () => void; onUpdated: (user: UserProfile) => void }) {
  ...
  return <div className="page-content"><PageHeading kicker="Account" title="Profile & Settings" subtitle="Customize how other players see you across NovaTable." />
    <section className="profile-card card-panel"><Avatar user={preview} /><div><h2>{displayName || user.displayName}</h2><p>@{user.username}</p><span>{user.email}</span><code>User ID · {user.id}</code></div><button className="secondary-button" onClick={onLogout}><LogOut size={15} />Logout</button></section>
...
function LobbyRoom({ user, lobby, decks, onLobby, onNotify, onLeave, onStart }: { user: UserProfile; lobby: Lobby; decks: Deck[]; onLobby: (lobby: Lobby) => void; onNotify: (message: string) => void; onLeave: () => void; onStart: (lobby: Lobby) => void }) {
  ...
  useEffect(() => { if (lobby.status === "in-game" && !enteredGame) { setEnteredGame(true); onStart(lobby); } }, [enteredGame, lobby, onStart]);
  ...
  return <main className="lobby-room-screen"><header className="lobby-room-top"><div className="brand-lockup"><img src="/novatable-logo.svg" alt="" /><strong>NovaTable</strong></div><div><span className="kicker">Private multiplayer lobby</span><h1>{lobby.name}</h1></div><div className="lobby-window-actions"><FullscreenButton /><button className="secondary-button" onClick={onLeave}><LogOut size={15} />Leave Lobby</button></div></header>
    <div className="lobby-room-body"><section className="lobby-stage"><div className="lobby-summary card-panel"><div><span className="privacy-badge"><LockKeyhole size={12} />{lobby.privacy}</span><strong>{lobby.format}</strong><span>{lobby.players.length} / {lobby.maxPlayers} players</span></div><div className="lobby-code"><span>Lobby code</span><strong>{lobby.code}</strong><button onClick={() => { void navigator.clipboard?.writeText(lobby.code); onNotify("Lobby code copied"); }} aria-label="Copy lobby code"><Copy size={14} /></button></div></div>
...
function LobbyCard({ lobby, onJoin }: { lobby: Lobby; onJoin: () => void }) { return <article className="lobby-card card-panel"><div className="lobby-card__art"><Gamepad2 size={23} /></div><div><span>{lobby.status === "waiting" ? "Open" : "In game"}</span><h3>{lobby.name}</h3><p>{lobby.description}</p><div>{lobby.tags.map((tag) => <i key={tag}>{tag}</i>)}</div></div><aside><strong>{lobby.format}</strong><span>{lobby.players.length} / {lobby.maxPlayers} players</span><button onClick={onJoin}>Join Lobby</button></aside></article>; }
function LobbyRow({ lobby, onJoin }: { lobby: Lobby; onJoin: () => void }) { return <button className="lobby-row" onClick={onJoin}><span><Gamepad2 size={15} /></span><div><strong>{lobby.name}</strong><small>{lobby.format} · {lobby.tags.join(" · ")}</small></div><b>{lobby.players.length}/{lobby.maxPlayers}</b><ChevronRight size={14} /></button>; }

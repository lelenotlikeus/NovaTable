...
import {
  ...
  KeyRound, MailCheck, ShieldCheck, Trophy, Zap
} from "lucide-react";
...
import { levelForXp, xpForLevel, type Deck, type DeckCardEntry, type Lobby, type LobbyFormat, type UserProfile } from "../../domain/platform/types";
...
import {
  ...
  updateDeck, updateProfile, beginTwoFactorSetup, changeAccountEmail,
  changeAccountPassword, disableTwoFactor, enableTwoFactor
} from "../../infrastructure/localPlatform";
import {
  onlineCreateLobby, onlineFindLobby, onlineJoinLobby, onlineLeaveLobby,
  onlineLobby, onlineLobbyAction, onlinePublicLobbies, honorLeaderboard, publicProfile
    ...
    <div className="platform-shell" style={{ "--nt-green": user.accentColor ?? "#62e6bb" } as React.CSSProperties}>
      <aside className="app-sidebar">
        ...
        <div className="sidebar-account">
          <Avatar user={user} /><div><strong>{user.displayName} <OfficialBadge badge={user.badge} /></strong><span>@{user.username} · Level {user.level ?? levelForXp(user.xp)}</span></div>
        </div>
      </aside>
...
function HomeView({ user, decks, revision, onCreate, onJoin, onBrowse, onNotify }: {
  ...
  return <div className="page-content home-page">
    ...
    <div className="home-grid">
      ...
      <section className="card-panel section-card"><SectionTitle title="Honor leaderboard" action="Real players only" />{leaders.map((leader, index) => <div className="honor-row" key={leader.id}><b>{index + 1}</b><PersonRow user={leader} /><strong>Lv. {leader.level ?? levelForXp(leader.xp)}</strong></div>)}{!leaders.length && <EmptyMini text="The leaderboard will appear after real players complete games." />}</section>
    </div>
  </div>;
}
...
function ProfileView({ user, onLogout, onUpdated }: { user: UserProfile; onLogout: () => void; onUpdated: (user: UserProfile) => void }) {
  ...
  const level = user.level ?? levelForXp(user.xp); const xp = user.xp ?? 0;
  const levelStart = xpForLevel(level); const levelEnd = xpForLevel(level + 1);
  return <div className="page-content"><PageHeading kicker="Account" title="Profile & Settings" subtitle="Customize how other players see you across NovaTable." />
    <section className="profile-card card-panel"><Avatar user={preview} /><div><h2>{displayName || user.displayName} <OfficialBadge badge={user.badge} /></h2><p>@{user.username} · Level {level} · Honor {user.honor ?? 0}</p><div className="profile-level"><span style={{ width: `${Math.max(0, Math.min(100, (xp - levelStart) / Math.max(1, levelEnd - levelStart) * 100))}%` }} /><small>{xp} XP · {levelEnd - xp} to level {level + 1}</small></div><div className="profile-stats"><span><Gamepad2 size={13} />{user.gamesPlayed ?? 0} played</span><span><Trophy size={13} />{user.gamesWon ?? 0} won</span></div><span>{user.email}</span><code>User ID · {user.id}</code></div><button className="secondary-button" onClick={onLogout}><LogOut size={15} />Logout</button></section>
    ...
    <SecuritySettings user={user} onUpdated={onUpdated} />
...
function SecuritySettings({ user, onUpdated }: { user: UserProfile; onUpdated: (user: UserProfile) => void }) {
  ...
  const [twoFactorPassword, setTwoFactorPassword] = useState(""); const [code, setCode] = useState(""); const [setup, setSetup] = useState<{ secret: string; otpauthUri: string } | null>(null);
  const [message, setMessage] = useState<string | null>(null); const [busy, setBusy] = useState(false);
  const run = async (operation: () => Promise<void>, success: string) => { setBusy(true); setMessage(null); try { await operation(); setMessage(success); } catch (cause) { setMessage(cause instanceof Error ? cause.message : String(cause)); } finally { setBusy(false); } };
  return <section className="account-security card-panel"><header><ShieldCheck size={19} /><div><strong>Account security</strong><p>Manage your sign-in details without leaving NovaTable.</p></div></header><div className="security-list">
    <form className="security-row" onSubmit={(event) => { event.preventDefault(); void run(async () => { onUpdated(await changeAccountEmail(user.id, email, emailPassword)); setEmailPassword(""); }, "Email updated"); }}><span className="security-row__icon"><MailCheck size={19} /></span><div className="security-row__copy"><strong>Sign-in email</strong><p>Change the address used to access your account.</p></div><div className="security-row__fields"><label>New email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label><label>Current password<input type="password" value={emailPassword} onChange={(event) => setEmailPassword(event.target.value)} autoComplete="current-password" required /></label></div><button className="secondary-button" disabled={busy}>Update email</button></form>
    <form className="security-row" onSubmit={(event) => { event.preventDefault(); if (newPassword !== confirmation) return setMessage("The new passwords do not match."); void run(async () => { await changeAccountPassword(user.id, currentPassword, newPassword); setCurrentPassword(""); setNewPassword(""); setConfirmation(""); }, "Password updated"); }}><span className="security-row__icon"><KeyRound size={19} /></span><div className="security-row__copy"><strong>Password</strong><p>Use at least 8 characters and keep it unique.</p></div><div className="security-row__fields security-row__fields--three"><label>Current<input type="password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" required /></label><label>New password<input type="password" minLength={8} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" required /></label><label>Confirm<input type="password" minLength={8} value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="new-password" required /></label></div><button className="secondary-button" disabled={busy}>Update password</button></form>
    <form className="security-row security-row--2fa" onSubmit={(event) => { event.preventDefault(); if (user.twoFactorEnabled) void run(async () => { onUpdated(await disableTwoFactor(twoFactorPassword, code)); setCode(""); setTwoFactorPassword(""); }, "Two-factor authentication disabled"); else if (setup) void run(async () => { onUpdated(await enableTwoFactor(twoFactorPassword, setup.secret, code)); setSetup(null); setCode(""); setTwoFactorPassword(""); }, "Two-factor authentication enabled"); else void run(async () => setSetup(await beginTwoFactorSetup(twoFactorPassword)), "Add the key to your authenticator, then enter its code"); }}><span className="security-row__icon"><ShieldCheck size={19} /></span><div className="security-row__copy"><strong>Two-factor authentication <i className={user.twoFactorEnabled ? "is-on" : ""}>{user.twoFactorEnabled ? "ON" : "OFF"}</i></strong><p>{user.twoFactorEnabled ? "Authenticator codes protect every new login." : "Add an authenticator code to every new login."}</p></div><div className="security-row__fields"><label>Current password<input type="password" value={twoFactorPassword} onChange={(event) => setTwoFactorPassword(event.target.value)} autoComplete="current-password" required /></label>{setup && <label>Setup key<code className="two-factor-secret">{setup.secret}</code></label>}{(setup || user.twoFactorEnabled) && <label>6-digit code<input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" pattern="\d{6}" required /></label>}</div><button className="secondary-button" disabled={busy}>{user.twoFactorEnabled ? "Disable 2FA" : setup ? "Enable 2FA" : "Set up 2FA"}</button>{setup && <small className="security-row__hint">Add this key manually to any TOTP authenticator app, then enter the generated code.</small>}</form>
  </div>{message && <div className={/updated|enabled|disabled|Add/.test(message) ? "profile-saved" : "inline-error"}>{message}</div>}</section>;
}
...
function OfficialBadge({ badge }: { badge?: UserProfile["badge"] }) { return badge ? <em className={`official-badge official-badge--${badge.toLowerCase()}`}>[{badge}]</em> : null; }
function ProfilePeek({ user }: { user: UserProfile }) { const level = user.level ?? levelForXp(user.xp); return <aside className="profile-peek"><header><Avatar user={user} /><div><strong>{user.displayName}</strong><span>@{user.username} <OfficialBadge badge={user.badge} /></span></div></header><div className="profile-peek__level"><Zap size={14} />Level {level}<small>{user.xp ?? 0} XP</small></div><div><span>{user.gamesPlayed ?? 0}<small>Games</small></span><span>{user.gamesWon ?? 0}<small>Wins</small></span><span>{user.honor ?? 0}<small>Honor</small></span></div>{user.bio && <p>{user.bio}</p>}</aside>; }
function ProfileLink({ user }: { user: UserProfile }) {
  const [open, setOpen] = useState(false); const [profile, setProfile] = useState<UserProfile | null>(null); const shown = profile ?? user;
  function toggle() { const next = !open; setOpen(next); if (next && !profile) void publicProfile(user.username).then((value) => value && setProfile(value)).catch(() => {}); }
  return <span className="profile-trigger"><button type="button" aria-expanded={open} onClick={toggle} onKeyDown={(event) => { if (event.key === "Escape") setOpen(false); }}>@{shown.username}</button>{open && <ProfilePeek user={shown} />}</span>;
}
function PersonRow({ user }: { user: UserProfile }) {
  return <div className="person-row"><Avatar user={user} /><div><strong>{user.displayName} <OfficialBadge badge={user.badge} /></strong><ProfileLink user={user} /></div><i className={`presence presence--${user.presence}`} /></div>;
}

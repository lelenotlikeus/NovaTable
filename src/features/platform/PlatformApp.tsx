...
function SecuritySettings({ user, onUpdated }: { user: UserProfile; onUpdated: (user: UserProfile) => void }) {
  ...
  return <section className="account-security card-panel"><header><ShieldCheck size={19} /><div><strong>Account security</strong><p>Manage your sign-in email, password and two-factor authentication.</p></div></header><div className="security-grid">
    <form onSubmit={(event) => { event.preventDefault(); void run(async () => { onUpdated(await changeAccountEmail(user.id, email, emailPassword)); setEmailPassword(""); }, "Email updated"); }}><MailCheck size={18} /><strong>Change email</strong><label>New email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label><label>Current password<input type="password" value={emailPassword} onChange={(event) => setEmailPassword(event.target.value)} autoComplete="current-password" required /></label><button className="secondary-button" disabled={busy}>Update email</button></form>
    <form onSubmit={(event) => { event.preventDefault(); if (newPassword !== confirmation) return setMessage("The new passwords do not match."); void run(async () => { await changeAccountPassword(user.id, currentPassword, newPassword); setCurrentPassword(""); setNewPassword(""); setConfirmation(""); }, "Password updated"); }}><KeyRound size={18} /><strong>Change password</strong><label>Current password<input type="password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" required /></label><label>New password<input type="password" minLength={8} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" required /></label><label>Confirm new password<input type="password" minLength={8} value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="new-password" required /></label><button className="secondary-button" disabled={busy}>Update password</button></form>
    <form onSubmit={(event) => { event.preventDefault(); if (user.twoFactorEnabled) void run(async () => { onUpdated(await disableTwoFactor(twoFactorPassword, code)); setCode(""); setTwoFactorPassword(""); }, "Two-factor authentication disabled"); else if (setup) void run(async () => { onUpdated(await enableTwoFactor(twoFactorPassword, setup.secret, code)); setSetup(null); setCode(""); setTwoFactorPassword(""); }, "Two-factor authentication enabled"); else void run(async () => setSetup(await beginTwoFactorSetup(twoFactorPassword)), "Add the key to your authenticator, then enter its code"); }}><ShieldCheck size={18} /><strong>Two-factor authentication</strong><p>{user.twoFactorEnabled ? "Enabled · codes are required when you sign in." : "Use any TOTP authenticator app for an extra sign-in check."}</p><label>Current password<input type="password" value={twoFactorPassword} onChange={(event) => setTwoFactorPassword(event.target.value)} autoComplete="current-password" required /></label>{setup && <><label>Authenticator setup key<code className="two-factor-secret">{setup.secret}</code></label><small>Add this setup key manually in Google Authenticator, Microsoft Authenticator, Authy or another TOTP app.</small></>}{(setup || user.twoFactorEnabled) && <label>6-digit code<input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" pattern="\d{6}" required /></label>}<button className="secondary-button" disabled={busy}>{user.twoFactorEnabled ? "Disable 2FA" : setup ? "Enable 2FA" : "Set up 2FA"}</button></form>
...
function PersonRow({ user }: { user: UserProfile }) {
  const [profile, setProfile] = useState<UserProfile | null>(null); const shown = profile ?? user;
  return <div className="person-row" tabIndex={0} onMouseEnter={() => { if (!profile) void publicProfile(user.username).then((value) => value && setProfile(value)).catch(() => {}); }}><Avatar user={shown} /><div><strong>{shown.displayName} <OfficialBadge badge={shown.badge} /></strong><span>@{shown.username}</span></div><i className={`presence presence--${shown.presence}`} /><ProfilePeek user={shown} /></div>;
}
